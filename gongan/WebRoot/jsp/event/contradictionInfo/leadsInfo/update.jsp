...
<html>
  ...
  <body>
  	<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  		<legend>修改情报线索信息</legend>
	</fieldset>
	<form class="layui-form" method="post" id="form1">
		<input type="hidden"  name="menuid" value="${menuid}">
		<input type="hidden"  name="id" value="${leadsInfo.id}" />
		<input type="hidden"  name="cdtid" value=${leadsInfo.cdtid }>
		<div class="layui-form-item">
	    	<label class="layui-form-label"><font color="red" size=2>*</font>线索标题</label>
	    	<div class="layui-input-inline" style="width:52.5%;">
	    		<input type="text" name="ltitle" value='${leadsInfo.ltitle }' lay-verify="required" lay-verType="tips" autocomplete="off" placeholder="请输入线索标题" class="layui-input"  lay-reqtext="线索标题不能为空">
	    	</div>
	  	</div>
	  	<div class="layui-form-item">
	    	...
	    	<div class="layui-input-inline" style="width:20%;">
				<select id="lpointto"  name="lpointto" lay-verify="required" lay-verType="tips">
			 		...
			 		<option value="来市" <c:if test="${leadsInfo.lpointto eq '来市'}"> selected</c:if>>来市</option>
			 		<option value="去锡" <c:if test="${leadsInfo.lpointto eq '去锡'}"> selected</c:if>>去锡</option>
			 		<option value="赴省" <c:if test="${leadsInfo.lpointto eq '赴省'}"> selected</c:if>>赴省</option>
			 		<option value="进京" <c:if test="${leadsInfo.lpointto eq '进京'}"> selected</c:if>>进京</option>
			 		<option value="其他" <c:if test="${leadsInfo.lpointto eq '其他'}"> selected</c:if>>其他</option>
			 	</select>
			</div>
	    	...
	    	<div class="layui-input-inline" style="width:20%;">
				<select id="lsource"  name="lsource" lay-verify="required" lay-verType="tips">
			 		<option value="">--请选择--</option>
			 		<option value="网上监测" <c:if test="${leadsInfo.lsource eq '网上监测'}"> selected</c:if>>网上监测</option>
			 		<option value="电话监控" <c:if test="${leadsInfo.lsource eq '电话监控'}"> selected</c:if>>电话监控</option>
			 		<option value="人力情报" <c:if test="${leadsInfo.lsource eq '人力情报'}"> selected</c:if>>人力情报</option>
			 		<option value="其他" <c:if test="${leadsInfo.lsource eq '其他'}"> selected</c:if>>其他</option>
			 	</select>
			</div>
	  	</div>
	  	<div class="layui-form-item layui-form-text">
		    ...
		    <div class="layui-input-inline" style="width:52.5%;">
		      <textarea placeholder="请输入内容" class="layui-textarea" name="lcontent" lay-verify="required" lay-verType="tips">${leadsInfo.lcontent }</textarea>
		    </div>
		</div>
	  	<div class="layui-form-item">
	  	...
	  	<div class="layui-form-item">
	    	...
	    	<div class="layui-input-inline" style="width:52.5%;">
				<select id="xscz"  name="xscz" lay-verify="required" lay-verType="tips">
			 		<option value="">--请选择--</option>
			 		<option value="已发生" <c:if test="${leadsInfo.xscz eq '已发生'}"> selected</c:if>>已发生</option>
			 		<option value="未发生" <c:if test="${leadsInfo.xscz eq '未发生'}"> selected</c:if>>未发生</option>
			 	</select>
			</div>
	  	</div>
	  	<div class="layui-form-item layui-form-text">
		    ...
		    <div class="layui-input-inline" style="width:52.5%;">
		      <textarea placeholder="请输入内容" class="layui-textarea" name="czqkgs" lay-verify="required" lay-verType="tips">${leadsInfo.czqkgs }</textarea>
		    </div>
		</div>
		<div class="layui-form-item layui-form-text">
		    <label class="layui-form-label">关联事件名称</label>
		    <div class="layui-input-inline" style="width:52.5%;z-index:1;">
		    	<input type="hidden" name="conneteventid" id="conneteventid" value="${leadsInfo.conneteventid }">
		    	<input type="text" id="conneteventname" name="conneteventname" value="${leadsInfo.conneteventname }" autocomplete="off" placeholder="请添加关联事件" class="layui-input" readonly="readonly">
		    </div>
		    <span style="z-index:2;position:absolute;margin-left:-60px;margin-top:6px;">
		    	<i class="layui-icon" style="font-size: 20px; color: #1E9FFF;" onclick="updateConnetevent();">&#xe64c;</i>
		    </span>
		</div>
		<div class="layui-form-item">
		    ...
		    <input type="hidden" name="attachments" id="attachments" class="layui-input" value="${leadsInfo.attachments }"/>
		</div>
		<div class="layui-form-item">
	...
	<script type="text/javascript">
		var fileidstr = "${leadsInfo.attachments }";
		var delidstr = "";
		...
		layui.use(['form', 'laydate','upload'], function(){
		    ...
		    form.on('submit(msgSub)', function(data){
			  ...
			  function add(){
			  	$("#form1").ajaxSubmit({
	              	url:		'<c:url value="/updateLeadsInfo.do"/>',
	              	dataType:	'json',
	              	async:  false,
		...
		function updateConnetevent(){
			if($('#conneteventid').val()!=0){
				var index = layui.layer.open({
					title : "修改引发涉稳事件",
					type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
					content : '<c:url value="getEventsInfo.do"/>?id=${leadsInfo.conneteventid }&menuid=${menuid }&page=qbxs',
					area: ['800', '800px'],
					maxmin: true,
				...
				var index = layui.layer.open({
					title : "添加引发涉稳事件",
					type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
					content : '<c:url value="/jsp/event/contradictionInfo/eventsInfo/add.jsp"/>?menuid=${menuid }&cdtid=${leadsInfo.cdtid}&fxdj=${param.fxdj }&page=qbxs',
					area: ['800', '800px'],
					maxmin: true,
					success : function(layero, index){
						setTimeout(function(){
