  ...
  <div class="showinfo">
  	<table width="100%" border="1" cellspacing="0" cellpadding="0">
		<tr height="40" bgcolor="#3598DC">
    		<td colspan="4"><span class="showinfo_title">风险基本信息</span></td>
  		</tr>
  		<tr>
	    	<td width="15%">入库单位</td>
	    	<td width="35%">${workInfo.rkdept }</td>
	    	<td width="15%">入库时间</td>
	    	<td width="35%">${workInfo.addtime }</td>
	    </tr>
	    <tr>
	    	<td width="15%">入库人</td>
	    	<td width="35%">${workInfo.addoperator }</td>
	    	<td width="15%">风险等级</td>
	    	<td width="35%">${workInfo.cdtlevel }</td>
	    </tr>
	    <tr>
	    	<td width="15%">风险类别</td>
	    	<td width="35%">${workInfo.cdttypename }</td>
	    	<td width="15%">处置结果</td>
	    	<td width="35%">${workInfo.cdtresult }</td>
	    </tr>
	    <tr>
			<td width="15%">事发地址</td>
			<td width="80%" colspan="3">${workInfo.sfdz }</td>
	    </tr>
	    <tr>
	    	<td width="15%">涉事人数</td>
	    	<td width="35%">${workInfo.ssrs }</td>
	    	<td width="15%">涉及金额</td>
	    	<td width="35%">${workInfo.sjje }</td>
	    </tr>
	    <tr>
			<td width="15%">主办部门</td>
			<td width="80%" colspan="3">${workInfo.sponsorname }</td>
	    </tr>
	    <tr>
			<td width="15%">协办部门</td>
			<td width="80%" colspan="3">${workInfo.assistdeptname }</td>
	    </tr>
		<tr>
			<td width="15%">风险名称</td>
			<td width="80%" colspan="3">${workInfo.cdtname }</td>
	    </tr>
	    <tr>
			<td width="15%">风险矛盾概况</td>
			<td width="80%" colspan="3">${workInfo.cdtcontent }</td>
	    </tr>
	</table>
	...
	<table width="100%" border="1" cellspacing="0" cellpadding="0" style="margin-top: 20px;">
	    ...
	    <tr>
	    	<td width="15%">反馈附件信息</td>
	    	<td width="80%" colspan="3">
	    		<c:if test="${not empty workInfo.fkfilenames}">
    				<c:set value="${fn:split(workInfo.fkfilenames,',') }" var="fkfilenames"></c:set>
    				<c:set value="${fn:split(workInfo.fkfileids,',') }" var="fkfileids"></c:set>
    				<c:forEach items="${fkfilenames}" var="fkfilename" varStatus="num">
	    				<div><a href="<c:url value='/downUpfile.do' />?fileid=${fkfileids[num.index] }" class="layui-table-link" style="color: blue;">${fkfilename }</a></div>
	    			</c:forEach>
    			</c:if>
			</td>
	    </tr>
