...
layui.use(['table'], function(){
	...
	table.render({
	    ...
	    cols: [[
	    	...
	    	{field:'etitle', title: '事件标题', width:200,align:"center"},
	    	{field:'sfbwd', title: '事发部位', width:200,align:"center"},
	    	{field:'sjgk', title: '事件概况',align:"center"}
	    ]],
	    page: true,
	    limit: 10
	});
	...
	table.on('toolbar(followTable)', function(obj){
		...
		switch(obj.event){
			case 'add':
				   var content='<c:url value="/jsp/event/contradictionInfo/eventsInfo/add.jsp"/>?menuid=${param.menuid }&cdtid=${param.cdtid}&workinfoid=${param.workinfoid}&page=${param.page}&fxdj=${param.fxdj}';
		    ...
		    case 'update':
			   ...
			   var id=datas[0].id;
			    var index = layui.layer.open({
							title : "修改引发涉稳事件",
							type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
							content : '<c:url value="getEventsInfo.do"/>?id='+id+'&menuid=${param.menuid }&page=${param.page}',
							area: ['800', '800px'],
							maxmin: true,
							success : function(layero, index){
								setTimeout(function(){
			 ...
			 case 'canel':
				...
				if(datas!=""){
				    ...
				    layer.confirm('确定删除此信息？', function(index){
						layer.close(index);
				        $.getJSON('<c:url value="cancelEventsInfo.do"/>?id='+id+'&menuid='+${param.menuid },{
							},function(data) {
								var str = eval('(' + data + ')');
								if (str.flag ==1) {		                          
							     	...
							     	if("${param.page}"=="feedback"){
							     		parent.parent.formSubmit('swsj','<c:url value="searchEventsInfo.do"/>');
							     	}
								}else{
									top.layer.msg("删除失败!");
								}
 	...
 	function formSubmit(){
		layui.table.reload('followTable', {
			page: {
				...
				parent.$('#swsjbutton').text("引发涉稳事件("+count+")");
