...
<html>
  ...
  <body>
  	<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  		<c:if test="${handleInfo.htype  eq 1}">
  			<legend id="typeName">修改领导批示</legend>
  		</c:if>
  		<c:if test="${handleInfo.htype  eq 2}">
  			<legend id="typeName">修改涉稳专报</legend>
  		</c:if>
  		<c:if test="${handleInfo.htype  eq 3}">
  			<legend id="typeName">修改维稳专报</legend>
  		</c:if>
  		<c:if test="${handleInfo.htype  eq 4}">
  			<legend id="typeName">修改涉稳风险交办处置建议</legend>
  		</c:if>
  		<c:if test="${handleInfo.htype  eq 5}">
  			<legend id="typeName">修改专题会议纪要</legend>
  		</c:if>
	</fieldset>
	<form class="layui-form" method="post" id="form1">
		<input type="hidden"  name="menuid" value="${menuid}">
		<input type="hidden"  name="id" value="${handleInfo.id}" />
		<input type="hidden"  name="cdtid" value=${handleInfo.cdtid }>
		<input type="hidden"  name="htype" value=${handleInfo.htype }>
		<div class="layui-form-item">
	    	...
	    	<div class="layui-input-inline" style="width:20%;">
				<input type="text" class="layui-input" id="htitle" name="htitle" value="${handleInfo.htitle }">
			</div>
	    	<c:if test="${handleInfo.htype  eq 1}">
  				<label class="layui-form-label" style="width:10%;">录入时间</label>
  			</c:if>
  			<c:if test="${handleInfo.htype  eq 2 or handleInfo.htype  eq 3}">
	  	...
	  	<div class="layui-form-item layui-form-text">
		    ...
		    <div class="layui-input-inline" style="width:52.5%;">
		      <textarea placeholder="请输入内容" class="layui-textarea" name="hcontent">${handleInfo.hcontent }</textarea>
		    </div>
		</div>
		<c:if test="${param.htype  eq 1}">
		  	<div class="layui-form-item">
		    	...
		    	<div class="layui-input-inline" style="width:20%;">
					<input type="text" class="layui-input" id="approveperson" name="approveperson" value="${handleInfo.approveperson }">
				</div>
		  	</div>
	  	</c:if>
	  	<c:if test="${handleInfo.htype  eq 4}">
			<div class="layui-form-item">
		    	...
		    	<div class="layui-input-inline" style="width:52.5%;">
		      		<input type="text" class="layui-input" id="receiver" name="receiver" value="${handleInfo.receiver }">
		    	</div>
		  	</div>
			<div class="layui-form-item layui-form-text">
			    <label class="layui-form-label">办理情况</label>
			    <div class="layui-input-inline" style="width:52.5%;">
			      <textarea placeholder="请输入内容" class="layui-textarea" name="hsituation">${handleInfo.hsituation }</textarea>
			    </div>
			</div>
			<div class="layui-form-item">
		    	<label class="layui-form-label">处置结果</label>
		    	<div class="layui-input-inline" style="width:52.5%;">
		      		<input type="text" class="layui-input" id="hresult" name="hresult" value="${handleInfo.hresult }">
		    	</div>
		  	</div>
		</c:if>
	  	<div class="layui-form-item">
		    ...
		    <div class="layui-input-block" style="width:50%">
		    	...
		    	<div class="layui-upload-list">
		    		<table class="layui-table">
		    			...
		    			<tbody id="filelist">
		    				<c:if test="${not empty handleInfo.filenames}">
			    				<c:set value="${fn:split(handleInfo.filenames,',') }" var="filenames"></c:set>
			    				<c:set value="${fn:split(handleInfo.fileids,',') }" var="fileids"></c:set>
		    ...
		    <input type="hidden" name="attachments" id="attachments" class="layui-input" value="${handleInfo.attachments }"/>
		</div>
		<div class="layui-form-item">
	...
	<script type="text/javascript">
		var fileidstr = "${handleInfo.attachments }";
		var delidstr = "";
		layui.use(['form', 'laydate','upload'], function(){
			...
			laydate = layui.laydate;
		  	...
		  	laydate.render({
			  elem: '#hdate',
			  value: '${handleInfo.hdate }'
			});
		    
		    if(${handleInfo.htype }==1){
			    //初始化领导级别
			    $.ajax({
					type:		'POST',
					...
					success:	function(data){
		          			$.each(data, function(num, item) {
		          				if(item.name=="${handleInfo.leaderlevel}"){
									$('#leaderlevel').append('<option value="' + item.name + '" selected>' + item.name + '</option>');
								}else{
		...
		function add(){
			...
			$("#form1").ajaxSubmit({
              	url:		'<c:url value="/updateHandleInfo.do"/>',
              	dataType:	'json',
              	async:  	false,
              	success:	function(data) {
                  	var obj = eval('(' + data + ')');
