...
layui.use(['table','form'], function(){
  ...
  table.render({
    ...
    cols: [[
    	...
    	{field:'identity', title: '身份', width:150},
    	{field:'personname', title: '姓名', width:200},
    	{field:'cardnumber', title: '证件号码', width:200} ,
        {field:'homeplace', title: '现住地', width:300} , 
    	{field:'telnumber', title: '手机号码', width:200},
    ]],
    page: true,
    limit: 10
  });
  ...
  table.on('toolbar(followTable)', function(obj){
   ...
   case 'add':
   		var index = layui.layer.open({
				...
				content : '<c:url value="/jsp/position/addpersonnel.jsp?positionid=${position.id}&menuid=${menuid }"/>',
				area: ['1000', '600px'],
				maxmin: true,
	...
	case 'cancel':
	   ...
	   var datas=JSON.parse(data);
	    if(datas!=""){
	   var id=datas[0].id;
	    layer.confirm('确定删除此人员情况？', function(index){
	        layer.close(index);
	        $.getJSON(locat+"/cancelPositionPersonnel.do?id="+id+"&menuid="+${param.menuid },{},function(data) {
				var str = eval('(' + data + ')');
	        	 if (str.flag ==1) {		                          
