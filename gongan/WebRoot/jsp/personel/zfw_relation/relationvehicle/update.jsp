...
<html>
  <head>
    ...
    <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  		<legend>修改交通工具</legend>
	</fieldset>
	<form class="layui-form" method="post" id="form1">
		<input type="hidden"  name="menuid" id="menuid"  value=0></input>
		<input type="hidden"  name="id" id="id"  value=${relationvehicle.id}></input>
		<input type="hidden"  name="personnelid" id="personnelid"  value=${relationvehicle.personnelid}></input>
	 ...
	 <div class="layui-form-item">
	   ...
	   <div class="layui-form-item">
	       	<div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			<input type="text" name="vehiclenum"  id="vehiclenum"  autocomplete="off" placeholder="请输入车辆号牌" class="layui-input" value="${relationvehicle.vehiclenum}">
	      		</div>
	       	</div>
	     </div>
	     <div class="layui-form-item">
	       	<div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			<input type="text" name="vehiclebrand"  id="vehiclebrand"  autocomplete="off" placeholder="请输入品牌型号" class="layui-input" value="${relationvehicle.vehiclebrand}">
	      		</div>
	       	</div>
	     </div>
	   <div class="layui-form-item">
	       	<div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			<input type="text" name="vehiclecolor"  id="vehiclecolor"  autocomplete="off" placeholder="请输入车辆颜色" class="layui-input" value="${relationvehicle.vehiclecolor}">
	      		</div>
	       	</div>
	     </div> 
	   ...
	   <div class="layui-form-item">
	       	<div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			...
	      			<option   value="0" <c:if test="${relationvehicle.isactivate eq '0'}"> selected</c:if>>否</option>
	      			         <option   value="1" <c:if test="${relationvehicle.isactivate eq '1'}"> selected</c:if>>是</option>
	      			</select>
	      		</div>
	       	</div>
	     </div> 
	   <div class="layui-form-item">
	       	<div class="layui-inline">
			    ...
			    <div class="layui-input-inline"  style="width:250px;">
			      <textarea placeholder="请输入内容" class="layui-textarea" name="memo">${relationvehicle.memo}</textarea>
			    </div>
		    </div>
		</div>   
		<div class="layui-form-item">
				...
				type:		'POST',
				 ...
				 success:	function(data){					  
				      ...
				      $("select[name^=gainorigin]").html(options);
				       $("#gainorigin").val("${relationvehicle.gainorigin}");
				   }
			});
	    $.ajax({
				type:		'POST',
				 ...
				 success:	function(data){					  
				      ...
				      $("select[name^=vehicletype]").html(options);
				       $("#vehicletype").val("${relationvehicle.vehicletype}");
				   }
			});
	   $.ajax({
				type:		'POST',
				 ...
				 success:	function(data){					  
				      ...
				      $("select[name^=vehicleorigin]").html(options);
				       $("#vehicleorigin").val("${relationvehicle.vehicleorigin}");
				   }
			});
       });
		layui.use(['form', 'laydate'], function(){
		   ...
		   form.on('submit(roleSub)', function(data){
             ...
             $("#form1").ajaxSubmit({
	              	url:		'<c:url value="/updaterelationvehicle.do"/>',
	              	dataType:	'json',
	              	async:  false,
	              	success:	function(data) {
	                  	var obj = eval('(' + data + ')');
