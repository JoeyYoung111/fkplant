 ...
 <body class="childrenBody layui-fluid">
				...
				<div class="layui-row" style="border-bottom: 1px solid #eee;padding: 15px;">
					<table class="layui-table" lay-even>
						...
						<tbody>
							...
							<tr>
								<td class="text-align-r my-text-nowarp">政治面貌：</td>
								<td colspan="3">${personnel.politicalposition}</td>
								<td class="text-align-r my-text-nowarp">宗教信仰：</td>
								<td colspan="2">${personnel.faith}</td>
							</tr>
							<tr>
								<td class="text-align-r my-text-nowarp">文化程度：</td>
								<td colspan="3">${personnel.education}</td>
							...
							<tr>
							 ...
							 <tr>
								<td class="text-align-r my-text-nowarp">初次吸毒时间：</td>
								<td colspan="3">${duextend.firsttime}</td>
								<td class="text-align-r my-text-nowarp">末次处罚时间：</td>
								<td colspan="2">${duextend.lasttime}</td>
							</tr>
							<tr>
								<td class="text-align-r my-text-nowarp">滥用毒品种类：</td>
					...
					<div class="right-child-content layui-tab-item layui-show"><!--人员属性标签 -->
					 ...
					 <table class="layui-table" lay-even>
								...
								<tbody>
									...
									<tr>
										<td class="text-align-r my-text-nowarp">评定依据：</td>
										<td  colspan="3"  id="controlstatus">${duextend.controlstatus }</td>
									</tr>
									<tr>
										<td class="text-align-r my-text-nowarp">其他特殊状况：</td>
										<td  colspan="3">${duextend.controlstatusmemo}</td>
									</tr>
									<tr>
										<td class="text-align-r my-text-nowarp">平安关爱对象：</td>
										<td>
										    <c:if test="${duextend.caredperson eq 1}">否</c:if>
										    <c:if test="${duextend.caredperson eq 2}">是</c:if>
										</td>
									...
									<div class="layui-form-item my-form-item">
										...
										<div class="my-input-block">
											...
											<div class="layui-col-md4 layui-col-md-offset1">
												<label>人户分离:<c:choose>
																	<c:when test="${fn:contains(yesorno,',1,')}">
																		是
																	</c:when>
																	<c:otherwise>
																		否
											...
											<div class="layui-col-md4 layui-col-md-offset1">
												<label>双向管控:<c:choose>
																	<c:when test="${fn:contains(yesorno,',2,')}">
																		是
																	</c:when>
																	<c:otherwise>
																		否
											...
											<div class="layui-col-md4 layui-col-md-offset1">
												<label>精神疾病患者:<c:choose>
																	<c:when test="${fn:contains(yesorno,',3,')}">
																		是
																	</c:when>
																	<c:otherwise>
																		否
											...
											<div class="layui-col-md4 layui-col-md-offset1">
												<label>病残人员:<c:choose>
																	<c:when test="${fn:contains(yesorno,',4,')}">
																		是
																	</c:when>
																	<c:otherwise>
																		否
											...
											<div class="layui-col-md4 layui-col-md-offset1">
												<label>三年内处罚人员:<c:choose>
																	<c:when test="${fn:contains(yesorno,',5,')}">
																		是
																	</c:when>
																	<c:otherwise>
																		否
	...
	<script>
	 $(document).ready(function(){
			    ...
			    var controlstatus = "${duextend.controlstatus}";
			    console.log(controlstatus);
               controlstatus=controlstatus.replace(/,/g, ";<br />");
                console.log(controlstatus);
               $("#controlstatus").html(controlstatus);	
		});
