...
<%--		</div>--%>
    ...
    function getCustomLabelList(){
	    ...
	    layui.treetable.render({
    		...
    		url: '<c:url value="/getAttributeLabelTreeByparentid1.do"/>?parentid='+$('#nodeid').val()+"&examineflag=-1&isfilter=0",//isfilter 是否做责任警种过滤 0-否 1-是
            page: false,
            cols: [
            [
                {type: 'radio'},
                ...
                {field: 'addoperator',align: 'center', title: '审核信息',width:220,templet: function (item) {
	  ...
	  $("#search").click(function () {
		...
		//原始加载  二级标签
         treetable.render({
            ...
            //toolbar: true,
            toolbar: '#toolbarDemo',   //监听头部工具栏事件
    		 url: '<c:url value="/getAttributeLabelTreeByparentid1.do"/>?parentid='+$('#nodeid').val()+"&examineflag=-1&isfilter=0",
            page: false,
            cols: [
            [
                {type: 'radio'},
...
<%--					    --%>
<%--			   }else{--%>
<%--				  top.layer.alert("请先选择审核哪条数据！");--%>
<%--			   }--%>

              var index = layui.layer.open({
					title : "二级标签审核",
					type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
					content : '<c:url value="/jsp/personel/personlabel/checklist.jsp?isfilter=0&menuid='+${param.menuid }+'&parentid='+$('#nodeid').val()+'"/>',//未审核标签不做警种过滤
				    area: ['900', '500px'],
					maxmin: true,
					success : function(layero, index){
						setTimeout(function(){
