...
<%--		     <select name="attributelabels" id="attributelabels"  xm-select="attributelabels" xm-select-skin="normal" xm-select-search=""  xm-select-show-count="3"> </select>--%>
			<div id="attributelabels" style="width:500px;"></div>
		</div>
		<br>
		<div class="layui-inline" style="width:212px;">
		...
		<div class="layui-inline" style="width:212px;">
			<select id="jointcontrollevel">
				<option value=""> 涉政级别: 全部</option>
			</select>
		</div>
		<div class="layui-inline" style="width:212px;">
			<input class="layui-input" type="text" id="country" placeholder=" 国籍：" autocomplete="off" >
		</div>
		<input type="hidden"  name="incontrollevel"    id="incontrollevel"  value="">
...
<%--		</div>--%>
		<div class="layui-inline">
			   <div id="jurisdictunit1" style="width:360px;"></div>
		</div>
...
<%--        });--%>
	...
	_zTreeSelectM2 = zTreeSelectM({
		...
		data: '<c:url value="/getChildrenLabelByParentid.do"/>?parentid='+9,
	    type: 'get',  //设置了长度    
	    selected: [],//默认值            
	    max: 100,//最多选中个数，默认5            
				...
				if(length==15){
				   ...
				   if(item.birthday!=null){
					   ...
					   if(birthday.length>0){
					   		age-=parseInt(birthday.substring(0,4));
					   		if(parseInt(birthday.substring(5,7).toString()+birthday.substring(8,10).toString())>parseInt(new Date().Format("MMdd")))age--;
					   }else age=''; 
				   }else age='';
				}
...
           }},
        ...
        {field:'jointcontrollevel', title: '涉政级别', width:90} ,
        {field:'pphone2', title: '人员类别', width:150,templet: function (item) {
               var str="";
               if(item.pphone2!=null&&item.pphone2!=""){
	               		var zslabel2s=item.pphone2.split(",");
	               		for(var i=0;i<zslabel2s.length;i++){
		               		if(zslabel2s[i]!=""){
			               		$.ajax({
									...
									success:	function(data){
										if(data.rootid==9){
											var labelstr=data.attributelabel.split("-");
											str+="<span><font color='red'>"+labelstr[0]+"&nbsp;&nbsp;</font></span>";
										}
									}
								});
		               		}
...
           }},
        ...
        {field:'country', title: '国籍', width:90} ,
        {field:'homeplace', title: '现居住地详址', width:180} ,
...
<%--               }--%>
               var str="";
               if(item.unitname2!=null&&item.unitname2!=""){
	               		var zslabel1s=item.unitname2.split(",");
	               		for(var i=0;i<zslabel1s.length;i++){
		               		if(zslabel1s[i]!=""){
			               		$.ajax({
									...
									async:      false,
									success:	function(data){
										if(data.id!=9)str+="<span><font color='red'>"+data.attributelabel+"&nbsp;&nbsp;</font></span>";
									}
								});
		               		}
	               		}
...
           }},
        ...
        {field:'updateoperator', title: '修改人', width:170}, 
   ...
   case 'update':
	   ...
	   var id=datas[0].id;
	    var index = layui.layer.open({
					...
					type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
					content : locat+'/getPersonnelExtendUpdate.do?id='+id+'&buttons='+'${param.buttons }'+'&personlabelid='+${param.personlabelid}+'&page=update_zhengbao&menuid='+${param.menuid },
					area: ['800', '750px'],
					maxmin: true,
					success : function(layero, index){
						setTimeout(function(){
...
<%--		});--%>
 ...
 function showinfoPersonnelExtend(id){
 	var index = layui.layer.open({
		title : thistitle+"详情",
		type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
		content : locat+'/getPersonnelExtendUpdate.do?id='+id+'&personlabelid='+${param.personlabelid}+'&memo='+thistitle+'&menuid='+${param.menuid}+'&page=showinfo_zhengbao',
		area: ['800', '750px'],
		maxmin: true,
		success : function(layero, index){
			setTimeout(function(){
