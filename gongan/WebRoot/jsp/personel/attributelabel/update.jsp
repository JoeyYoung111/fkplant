...
<html>
  ...
  <body>
  	<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  		<legend>修改人员属性标签</legend>
	</fieldset>
	<form class="layui-form" method="post" id="form1">
		<input type="hidden"  name="menuid" value=${menuid }>
		<input type="hidden"  name="id" value=${attributelabel.id }>
		<div class="layui-form-item">
	       ...
	       <label class="layui-form-label"><font color="red" size=2>*</font>属性标签</label>
	    	<div class="layui-input-inline">
	      		<input type="text" name="attributelabel" lay-verify="required" autocomplete="off" placeholder="请输入属性标签" 
	      		value="${attributelabel.attributelabel}"  class="layui-input"  lay-reqtext="请输入属性标签">
	      	</div>
	    </div>
...
<%--	          </div>--%>
	 	 ...
	 	 <div class="layui-form-item">
		   <label class="layui-form-label">标签描述</label>
	          <div class="layui-input-inline">
		      			<textarea class="layui-textarea" rows=2 name="memo"   style="width:660px;">${attributelabel.memo}</textarea>
			    </div>
		 </div>
	 	<div class="layui-form-item">
	...
	<script type="text/javascript">
	 $(document).ready(function(){
	  // alert(${attributelabel.parentid});
	    if(${attributelabel.parentid}!="0"){
	        $("#department").hide();
	     }
     });
		  layui.config({
		...
		layui.use(['form', 'layedit','treeSelect','zTreeSelectM'], function(){
		  ...
		  layedit = layui.layedit;
		   ...
		   if(${attributelabel.parentid}=="0"){
	        var _zTreeSelectM1 = zTreeSelectM({
			    ...
			    type: 'get',  //设置了长度    
			    selected: [${attributelabel.departmentid}],//默认值           
			    max: 100,//最多选中个数，默认5            
		 ...
		 treeSelect.render({
		        elem: '#parentid',
		        data: '<c:url value="/getCustomLabelTreeSelect.do"/>',
		        ...
		        // 加载完成后的回调函数
		        success: function (d) {
		           treeSelect.checkNode('parentid', '${attributelabel.parentid}');
		  ...
		  form.on('submit(roleSub)', function(data){
	         $("#form1").ajaxSubmit({
	              	url:		'<c:url value="/updateattributelabel.do"/>',
	              	dataType:	'json',
	              	async:  false,
