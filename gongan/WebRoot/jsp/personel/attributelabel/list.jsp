...
<div class="demoTable">
   ...
   <script type="text/html" id="toolbarDemo">
     ...
     <button class="layui-btn layui-btn-sm" lay-event="examine"><i class="layui-icon layui-icon-username"></i>审核</button>
   </script>
 </div>
</blockquote>
...
layui.config({
       base: "<c:url value="/layui/lay/modules/"/>"
        ...
        treetable.render({
            ...
            [
                {type: 'radio'},
                {field: 'attributelabel', width: 250,title: '属性标签'},
                ...
                {field: 'departname', align: 'center', title: '责任部门'},
                {field: 'addoperator',align: 'center', title: '建档人',width:120},
                {field: 'addtime', width: 180, align: 'center', title: '建档时间'},
                {field: 'examineman', align: 'center',title: '审核人',width:120},
                {field: 'examinetime', width: 180, align: 'center', title: '审核时间'},
                {field: 'right', title: '操作',align: 'center', toolbar: '#auth-state',width: 200}
        ...
        table.on('toolbar(authTreeTable)', function (obj) {
    		//var checkStatus = table.checkStatus('authTreeTable');
			 ...
			 if(obj.event === 'add'){
			   ...
			   var parentlabel =datas.length==0? '无' : datas[0].attributelabel;
			   var index = layui.layer.open({
					title : "添加人员属性标签",
					type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
          ...
          if(obj.event === 'examine'){
	           ...
	           var datas=JSON.parse(data);
				   if(datas!=""){
					    var examineflag=datas[0].examineflag;
					    var id=datas[0].id;
					    if(examineflag==0){
						    var index = layui.layer.open({
							title : "人员标签审核",
