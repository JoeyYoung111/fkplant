...
<html>
  ...
  <body>
   <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  		<legend>修改社会关系</legend>
	</fieldset>
	<form class="layui-form" method="post" id="form1" onsubmit="return false;">
		<input type="hidden"  name="menuid" id="menuid"  value=0></input>
		<input type="hidden"  name="id" id="id"  value=${socialrelations.id}></input>
		<input type="hidden"  name="personnelid" id="personnelid"  value=${socialrelations.personnelid}></input>
	    ...
	    <div class="layui-form-item">
	       <div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			<input type="text" name="cardnumber"  autocomplete="off" placeholder="请输入身份证号" class="layui-input" lay-verify="required|Iscardnumber" value="${socialrelations.cardnumber}">
	      		</div>
	       	</div>
	       	<div class="layui-inline">
	    		<label class="layui-form-label">姓名</label>
	    		<div class="layui-input-inline">
	      			<input type="text" name="personname"  autocomplete="off" placeholder="请输入姓名" class="layui-input" value="${socialrelations.personname}">
	      		</div>
	       	</div>
	     </div> 
	      <div class="layui-form-item">
	       <div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			<input type="text" name="homeplace"  autocomplete="off" placeholder="请输入现居地" class="layui-input" value="${socialrelations.homeplace}">
	      		</div>
	       	</div>
	       	<div class="layui-inline">
	    		<label class="layui-form-label">工作单位</label>
	    		<div class="layui-input-inline">
	      			<input type="text" name="workplace"  autocomplete="off" placeholder="请输入工作单位" class="layui-input" value="${socialrelations.workplace}">
	      		</div>
	       	</div>
	     </div> 
	     <div class="layui-form-item">
	       <div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			<input type="text" name="telnumber1"  autocomplete="off" placeholder="请输入联系电话1" class="layui-input" value="${socialrelations.telnumber1}">
	      		</div>
	       	</div>
	       	<div class="layui-inline">
	    		<label class="layui-form-label">联系电话2</label>
	    		<div class="layui-input-inline">
	      			<input type="text" name="telnumber2"  autocomplete="off" placeholder="请输入联系电话2" class="layui-input" value="${socialrelations.telnumber2}">
	      		</div>
	       	</div>
	     </div> 
	     <div class="layui-form-item">
	       <div class="layui-inline">
	    		...
	    		<div class="layui-input-inline">
	      			<input type="text" name="telnumber3"  autocomplete="off" placeholder="请输入联系电话3" class="layui-input" value="${socialrelations.telnumber3}">
	      		</div>
	       	</div>
	     </div> 
	     <br><br>   
				...
				type:		'POST',
				 ...
				 success:	function(data){					  
				      ...
				      $("select[name^=gainorigin]").html(options);
				       $("#gainorigin").val("${socialrelations.gainorigin}");
				   }
			});
		 $.ajax({
				type:		'POST',
				 ...
				 success:	function(data){					  
				      ...
				      $("select[name^=relationtype]").html(options);
				       $("#relationtype").val("${socialrelations.relationtype}");
				   }
			});
			...
			$.ajax({
					type:		'POST',
					 ...
					 success:	function(data){					  
					      ...
					      $("#appellation").val("${socialrelations.appellation}");
					      
						  if($("#appellation").val()=="其他"){
							  $('#memoshow').show();
						      $("#memo").val("${socialrelations.memo}");
						  }
					   }
				});
       });
		layui.use(['form', 'laydate'], function(){
           ...
           form.verify({
			 ...
			 Iscardnumber:function(value,item){
		  			...
		  			var msg="";
		  			if(value!="${socialrelations.cardnumber}"){
			  			$.ajax({
							type:	'post',
							url:	'<c:url value="/getsocialrelationscount.do"/>',
							data:	{
										personnelid :  ${socialrelations.personnelid},
										cardnumber: 	value
									},
							dataType:	'json',
							async:		false,
		   ...
		   form.on('submit(roleSub)', function(data){
             ...
             $("#form1").ajaxSubmit({
	              	url:		'<c:url value="/updatesocialrelations.do"/>',
	              	dataType:	'json',
	              	async:  false,
	              	success:	function(data) {
	                  	var obj = eval('(' + data + ')');
