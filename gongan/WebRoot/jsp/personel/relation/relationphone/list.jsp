...
$(function(){if('main'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
  ...
  table.render({
    ...
    url: '<c:url value="/searchrelationphone.do?personnelid="/>'+${param.personnelid},
    method:'post',
    toolbar: '#toolbarButton',
    cols: [[
        {field:'id',type:'radio',fixed:'true',align:"center"},
        {field:'telbrand', title: '品牌型号', align:"center"},
        {field:'imei', title: 'IMEI', align:"center"},
        {field:'mac', title: '设备MAC', align:"center"},
			...
			switch(obj.event){
				case 'add':
			  		var index = layui.layer.open({
						...
						content : '<c:url value="/jsp/personel/relation/relationphone/add.jsp?personnelid="/>'+${param.personnelid},
						area: ['700px', '600px'],
						maxmin: true,
				...
				case 'update':
					...
					if(datas!=""){
					  	var id=datas[0].id;
					   	var index = layui.layer.open({
							title : "修改使用手机",
							type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
							content : locat+'/getrelationphonebyid.do?id='+id,
							area: ['700px', '600px'],
							maxmin: true,
							success : function(layero, index){
							}
				  ...
				  if(datas!=""){
						...
						layer.confirm('确定删除此信息？', function(index){
					      layer.close(index);
					      $.getJSON(locat+"/cancelrelationphone.do?id="+id+'&menuid=0',{},function(data) {
							 var str = eval('(' + data + ')');
					      	 if (str.flag ==1) {		                          
