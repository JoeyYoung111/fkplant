...
$(function(){if('main'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
  ...
  table.render({
    ...
    url: '<c:url value="/searchrelationhouse.do?personnelid="/>'+${param.personnelid},
    method:'post',
    toolbar: '#toolbarButton',
    cols: [[
        {field:'id',type:'radio',fixed:'true',align:"center"},
        {field:'housetype', title: '房产类型', width:100,align:"center"},
        {field:'houseorigin', title: '房产来源', width:100,align:"center"},
        {field:'houseaddress', title: '房产地址',align:"center"} ,
        {field:'liveflag', title: '是否落脚居住',align:"center",width:120,templet: function (item) {
            if (item.liveflag=="1") {
               return "<span>是</span>";
            }else if(item.liveflag=="0"){
                return "<span>否</span>";
            }
         }}, 
			...
			switch(obj.event){
				case 'add':
			  		var index = layui.layer.open({
						...
						content : '<c:url value="/jsp/personel/relation/relationhouse/add.jsp?personnelid="/>'+${param.personnelid},
						area: ['600px', '500px'],
						maxmin: true,
				...
				case 'update':
					...
					if(datas!=""){
					  	var id=datas[0].id;
					   	var index = layui.layer.open({
							title : "修改房产信息",
							type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
							content : locat+'/getrelationhousebyid.do?id='+id,
							area: ['600px', '500px'],
							maxmin: true,
							success : function(layero, index){
							}
				  ...
				  if(datas!=""){
						...
						layer.confirm('确定删除此信息？', function(index){
					      layer.close(index);
					      $.getJSON(locat+"/cancelrelationhouse.do?id="+id+'&menuid=0',{},function(data) {
							 var str = eval('(' + data + ')');
					      	 if (str.flag ==1) {		                          
