...
$(function(){if('main'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
  ...
  table.render({
    ...
    url: '<c:url value="/searchrelationtelnumber.do?personnelid="/>'+${param.personnelid},
    method:'post',
    toolbar: '#toolbarButton',
    cols: [[
        {field:'id',type:'radio',fixed:'true',align:"center"},
        {field:'telnumber', title: '手机号码（PN）', align:"center"},
        {field:'imsi', title: 'IMSI', align:"center"},
        {field:'operator', title: '运营商', align:"center"},
        {field:'belongarea', title: '归属地', align:"center"},
        {field:'gainorigin', title: '获取来源', align:"center"},
        {field:'knownlabel', title: '手机落实标签', align:"center"},
			...
			switch(obj.event){
				case 'add':
			  		var index = layui.layer.open({
						...
						content : '<c:url value="/jsp/personel/relation/relationtelnumber/add.jsp?personnelid="/>'+${param.personnelid},
						area: ['750px', '700px'],
						maxmin: true,
				...
				case 'update':
					...
					if(datas!=""){
					  	var id=datas[0].id;
					   	var index = layui.layer.open({
							title : "修改手机号码",
							type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
							content : locat+'/getrelationtelnumberbyid.do?id='+id,
							area: ['750px', '700px'],
							maxmin: true,
							success : function(layero, index){
							}
				  ...
				  if(datas!=""){
						...
						layer.confirm('确定删除此信息？', function(index){
					      layer.close(index);
					      $.getJSON(locat+"/cancelrelationtelnumber.do?id="+id+'&menuid=0',{},function(data) {
							 var str = eval('(' + data + ')');
					      	 if (str.flag ==1) {		                          
