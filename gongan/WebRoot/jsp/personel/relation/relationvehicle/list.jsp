...
$(function(){if('main'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
  ...
  table.render({
    ...
    url: '<c:url value="/searchrelationvehicle.do?personnelid="/>'+${param.personnelid},
    method:'post',
    toolbar: '#toolbarButton',
    cols: [[
        {field:'id',type:'radio',fixed:'true',align:"center"},
        {field:'gainorigin', title: '获取来源', align:"center"},
        {field:'vehicletype', title: '车辆类别', align:"center"},
        {field:'vehicleorigin', title: '车辆来源', align:"center"},
        {field:'vehiclenum', title: '车辆号牌', align:"center"},
        {field:'vehiclebrand', title: '品牌型号', align:"center"},
        {field:'vehiclecolor', title: '车辆颜色', align:"center"},
        ...
        {field:'memo', title: '备注信息', width:150,align:"center"} ,
			...
			switch(obj.event){
				case 'add':
			  		var index = layui.layer.open({
						...
						content : '<c:url value="/jsp/personel/relation/relationvehicle/add.jsp?personnelid="/>'+${param.personnelid},
						area: ['750px', '700px'],
						maxmin: true,
				...
				case 'update':
					...
					if(datas!=""){
					  	var id=datas[0].id;
					   	var index = layui.layer.open({
							title : "修改交通工具",
							type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
							content : locat+'/getrelationvehiclebyid.do?id='+id,
							area: ['750px', '700px'],
							maxmin: true,
							success : function(layero, index){
							}
				  ...
				  if(datas!=""){
						...
						layer.confirm('确定删除此信息？', function(index){
					      layer.close(index);
					      $.getJSON(locat+"/cancelrelationvehicle.do?id="+id+'&menuid=0',{},function(data) {
							 var str = eval('(' + data + ')');
					      	 if (str.flag ==1) {		                          
