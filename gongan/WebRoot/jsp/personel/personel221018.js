...
function showJqRelDetail(jqId, duIds, changIds, peiIds) {
    ...
    if(changIds && changIds.trim()){
        content += '<h3 style="color:#1E9FFF;border-bottom:2px solid #1E9FFF;padding-bottom:5px;">关联涉娼记录</h3>';
...
function loadDuDetails(duIds, container) {
    ...
    html += '<thead><tr><th>ID</th><th>处罚时间</th><th>赌博方式</th><th>处罚结果</th><th>采集来源</th></tr></thead><tbody>';
...
function loadChangDetails(changIds, container) {
    var html = '<table class="layui-table" lay-size="sm">';
    html += '<thead><tr><th>ID</th><th>处罚时间</th><th>卖淫方式</th><th>处罚结果</th><th>采集来源</th></tr></thead><tbody>';
...
function loadPeiDetails(peiIds, container) {
    var html = '<table class="layui-table" lay-size="sm">';
    html += '<thead><tr><th>ID</th><th>处罚时间</th><th>陪侍地点</th><th>处罚结果</th><th>采集来源</th></tr></thead><tbody>';
        ...
        $.ajax({
            ...
            success: function(data){
                if(data){
                    html += '<tr>';
                    html += '<td>' + (data.id || '-') + '</td>';
                    html += '<td>' + (data.pei_chsj || '-') + '</td>';
                    html += '<td>' + (data.pei_psdd || '-') + '</td>';
                    html += '<td>' + (data.pei_cfjg || '-') + '</td>';
                    html += '<td>' + (data.collectSource || '-') + '</td>';
                    html += '</tr>';
                    loadedCount++;
                }
