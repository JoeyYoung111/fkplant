...
           }},
        ...
        {field:'parameter1', title: '审核时间', width:130} ,
        {field:'parameter2', title: '审核结果', width:120} ,
        {field:'parameter3', title: '银行机构代码', width:120} ,
        {field:'parameter4', title: '银行名称', width:120} ,
        {field:'parameter5', title: '银行所在地', width:120} ,
        {field:'parameter6', title: '联系电话', width:120} ,
        {field:'parameter7', title: '业务类型', width:120} ,
	...
	case 'cancel':
	   ...
	   if(datas.length>0){
		   layer.confirm('确定删除勾选信息？', function(index){
		       ...
		       $.getJSON(locat+"/cancelTrailInformation.do?ids="+ids.join(",")+"&trailtype=1&menuid="+${param.menuid },{},function(data) {
					var str = eval('(' + data + ')');
		        	 if (str.flag ==1) {		                          
...
<%--	   	break;--%>
		   ...
		   var index = layui.layer.open({
				 title : "导入全国银行信息",
				 type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
				 content : '<c:url value="/jsp/trail/import.jsp?trailtype=1"/>',
				  area: ['600', '650'],
				 maxmin: true,
				 offset:['30'], 
				 btn:['导入','关闭'],
