...
           }},
        ...
        {field:'parameter2', title: '航班号', width:100} ,
        {field:'parameter3', title: '起飞机场', width:100} ,
        {field:'parameter4', title: '到达机场', width:120} ,
        {field:'parameter5', title: '出发时间', width:120} ,
        {field:'parameter6', title: '行动名称', width:120} ,
        {field:'parameter7', title: '航空公司', width:150} ,
        {field:'parameter8', title: '订票日期', width:100} ,
        {field:'parameter9', title: '证件类型', width:120} ,
        {field:'parameter10', title: '证件号码', width:150} ,
        {field:'parameter11', title: '英文名', width:100} ,
        {field:'parameter12', title: '英文姓', width:100} ,
	...
	case 'cancel':
	   ...
	   if(datas.length>0){
		   layer.confirm('确定删除勾选信息？', function(index){
		       ...
		       $.getJSON(locat+"/cancelTrailInformation.do?ids="+ids.join(",")+"&trailtype=4&menuid="+${param.menuid },{},function(data) {
					var str = eval('(' + data + ')');
		        	 if (str.flag ==1) {		                          
...
<%--	   	break;--%>
		   ...
		   var index = layui.layer.open({
				 title : "导入全国民航订票信息",
				 type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
				 content : '<c:url value="/jsp/trail/import.jsp?trailtype=4"/>',
				  area: ['600', '650'],
				 maxmin: true,
				 offset:['30'], 
				 btn:['导入','关闭'],
