...
layui.use(['table','form'], function(){
  ...
  table.render({
    ...
    cols: [[
        ...
        {field:'cardnumber', title: '出生年月', width:100,templet: function (item) {
          		var age="";
				var cardnumber=item.cardnumber.toString();
				var length=cardnumber.length;
				if(length==15){
					age="19"+cardnumber.substring(6,12);
				}else if(length==18){
					age=cardnumber.substring(6,14);
				}else{
				   age=''; 
				}
...
           }},
        ...
        {field:'parameter1', title: '证件号码', width:120} ,
        {field:'parameter2', title: '出入日期', width:100} ,
        {field:'parameter3', title: '出入时间', width:100} ,
        {field:'parameter4', title: '交通方式', width:100} ,
        {field:'parameter5', title: '交通工具', width:100} ,
        {field:'parameter6', title: '人员类别', width:120} ,
        {field:'parameter7', title: '前往归来国家', width:120} ,
        {field:'parameter8', title: '国籍', width:120} ,
        {field:'parameter9', title: '证件类型', width:120} ,
	...
	case 'cancel':
	   ...
	   if(datas.length>0){
		   layer.confirm('确定删除勾选信息？', function(index){
		       ...
		       $.getJSON(locat+"/cancelTrailInformation.do?ids="+ids.join(",")+"&trailtype=6&menuid="+${param.menuid },{},function(data) {
					var str = eval('(' + data + ')');
		        	 if (str.flag ==1) {		                          
...
<%--	   	break;--%>
		   ...
		   var index = layui.layer.open({
				 title : "导入全国出入境信息",
				 type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
				 content : '<c:url value="/jsp/trail/import.jsp?trailtype=6"/>',
				  area: ['600', '650'],
				 maxmin: true,
				 offset:['30'], 
				 btn:['导入','关闭'],
