...
           }},
        ...
        {field:'parameter2', title: '驾驶证号码', width:120} ,
        {field:'parameter3', title: '号牌号码', width:120} ,
        {field:'parameter4', title: '号牌种类', width:120} ,
        {field:'parameter5', title: '违章时间', width:120} ,
        {field:'parameter6', title: '违法地点', width:120} ,
        {field:'parameter7', title: '处理时间', width:120} ,
        {field:'parameter8', title: '处理机构', width:150} ,
	...
	case 'cancel':
	   ...
	   if(datas.length>0){
		   layer.confirm('确定删除勾选信息？', function(index){
		       ...
		       $.getJSON(locat+"/cancelTrailInformation.do?ids="+ids.join(",")+"&trailtype=5&menuid="+${param.menuid },{},function(data) {
					var str = eval('(' + data + ')');
		        	 if (str.flag ==1) {		                          
...
<%--	   	break;--%>
		   ...
		   var index = layui.layer.open({
				 title : "导入全国机动车违章信息",
				 type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
				 content : '<c:url value="/jsp/trail/import.jsp?trailtype=5"/>',
				  area: ['600', '650'],
				 maxmin: true,
				 offset:['30'], 
				 btn:['导入','关闭'],
