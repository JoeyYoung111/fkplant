...
<body>
<form class="layui-form" method="post" id="form_updatecheck">

	<fieldset class="layui-elem-field layui-field-title" style="margin-top: 5px;">
		<legend>单位核查修改</legend>
	</fieldset>
	
	<input type="hidden" name="id" id="id" value="${id}" />
	<input type="hidden" name="companyid" id="companyid" value="${check.companyid }" />
	...
	<div class="layui-form-item">
      	<div class="layui-inline">
			...
			<div class="layui-input-inline">
				<input type="text" class="layui-input" style="width:670px;" name="checkdate" id="checkdate" value="${check.checkdate}" autocomplete="off" />
			</div>
		</div>
	</div>
	...
	<div class="layui-form-item">
		<div class="layui-inline">
			...
			<div class="layui-input-inline">
				<input type="text" class="layui-input" id="resultsummary" name="resultsummary" value="${check.resultsummary}" style="width:670px;" />
			</div>
		</div>
	</div>
	
	<div class="layui-form-item">
		<label class="layui-form-label"><font color="red" size=2>*</font>附件</label>
	...
	<div class="layui-form-item layui-form-text">
		<label class="layui-form-label">备注信息</label>
	 	<div class="layui-input-inline" style="width:670px;">
	 		<textarea name="memo" id="memo" class="layui-textarea">${check.memo}</textarea>
	 	</div>
	</div>

	<div class="layui-form-item">
	    <div class="layui-input-block">
	      <button type="submit" class="layui-btn" lay-submit="" lay-filter="UPDATECheck">立即提交</button>
	      <button type="reset" class="layui-btn layui-btn-primary" id="UPDATECheckReset">重置</button>
	    </div>
  	</div>
...
layui.use(['form','upload','laydate'],function(){
	...
	form.on('submit(UPDATECheck)',function(data){
		var remainStateidstr = $("#remainStateid").val();
		if(choose_files < 1){
			if(remainStateidstr != ""){
	...
	function updateCheck(){
		...
		$("#form_updatecheck").ajaxSubmit({
			url:		'<c:url value="/updateCheck.do"/>',
			dataType:	'json',
			async:		true,
			success:	function(data) {
				var obj = eval('(' + data + ')');
