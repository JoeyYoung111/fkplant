...
<div class="demoTable">
 <div class="layui-inline">
    <input class="layui-input" name="id" id="btnname" autocomplete="off" value=" 按钮名称：" onfocus="if(this.value == ' 按钮名称：'){ this.value = '';}" onblur="if(this.value =='') {this.value = ' 按钮名称：'}">
 </div>
...
layui.config({
       base: "<c:url value="/layui/lay/modules/"/>"
        ...
        treetable.render({
            ...
            //url: 'json/menus.json',
             url: '<c:url value="/getAppButtonTreeTable.do"/>',
            page: false,
            cols: [
            [
                {type: 'radio'},
                {field: 'btnname', minWidth: 100, title: '菜单名称'},
                {field: 'orderno', width: 80, align: 'center', title: '排序号'},
                {field: 'right', title: '操作', toolbar: '#auth-state',width: 200}
  ...
  table.on('toolbar(authTreeTable)', function(obj){
   ...
   var index = layui.layer.open({
				...
				content : "<c:url value="/jsp/basic/appbutton/add.jsp?menuid=${param.menuid }"/>",
				area: ['800', '800px'],
				maxmin: true,
  ...
  table.on('tool(authTreeTable)', function(obj){
    ...
    if(obj.event === 'del'){
      layer.confirm('确定删除此信息？', function(index){
        layer.close(index);
        $.getJSON(locat+"/cancelAppButton.do?id="+id+"&menuid="+${param.menuid },{
			},function(data) {
			var str = eval('(' + data + ')');
        	 if (str.flag ==1) {		                          
     ...
     var index = layui.layer.open({
				title : "修改菜单",
				type : 2,  //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
				content : locat+'/getAppButtonUpdate.do?id='+id+"&menuid="+${param.menuid },
				area: ['800', '800px'],
				maxmin: true,
				success : function(layero, index){
					setTimeout(function(){
