...
//新增页面 文件表单初始化样式
function initinputfile2(){
	$("#myFile2").fileinput({
        language : 'zh',//设置语言
        uploadUrl : locat+"/newuploadfile2.do",
        uploadAsync:true, 
        showBrowse:true,
		showCaption: true, 
		...
		allowedFileExtensions : ['mp4'],	//接收文件后缀
        ...
        document.getElementById("fileidstr2").value=document.getElementById("fileidstr2").value+res.success+",";            
    });
}
//初始化时触发
$('#myFile2').on('filereset', function(event) {
...
$('#myFile2').on('fileloaded', function(event, file, previewId, index, reader) {
    console.log("fileloaded");
    document.getElementById("filestatus2").value=document.getElementById("filestatus2").value+previewId+",";
 
});

$('#myfile2').on('change',function(event){
	document.getElementById('deleteidstr2').value=document.getElementById('fileidstr2').value;
});
//清空时执行		
$('#myFile2').on('filecleared', function(event) {
		console.log("filecleared");
});
//已有图片删除前执行
$('#myFile2').on('filepredelete', function(event, key, jqXHR, data) {	
	document.getElementById('deleteidstr2').value=document.getElementById('deleteidstr2').value+key+",";
	var fileidstr=document.getElementById('fileidstr2').value;
	...
	document.getElementById('fileidstr2').value=bb;
	}
...
//未上传图片删除时执行
$('#myFile2').on('filepreremove', function(event, id, index) {
	var fileidstr=document.getElementById('filestatus2').value;
	...
	document.getElementById('filestatus2').value=bb;
	}
			...
			for ( var i = 0; i < reData.length; i++) { 
				...
				if(array_element.filename.indexOf("gif")>0||
					...
					array_element.filename.indexOf("bmp")>0){ 				
					 preList[i]= "<img src='../../upload/qualityproblem/"+array_element.fileallname+"' class=\"file-preview-image\">"; 
...
function showFiles2(fileidstr){ 
   ...
   // 预览图片json数据组 
	...
	for(var i = 0; i < names.length-1; i++){
		...
		$("#myFile2").fileinput({
            language : 'zh',
            uploadUrl : locat+"/newuploadfile.do",
            uploadAsync:true, 
            showBrowse:true,
			showCaption: true, 
			showUpload: false,//是否显示上传按钮 
