...
-- 3. 新增：新警情案件轮询提示表
-- =================================================================
DROP TABLE IF EXISTS `p_case_police_message_t`;
CREATE TABLE `p_case_police_message_t` (
                                           `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
                                           `message_type` varchar(50) NOT NULL COMMENT '消息类型：aj-案件，jj-警情',
                                           `jjbh` varchar(100) DEFAULT NULL COMMENT '案件编号/接警编号',
                                           ...
                                           `sldw` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '受理单位code',
                                           `department_id` int(11) NOT NULL COMMENT '处理单位部门ID240-263',
                                           `department_name` varchar(200) DEFAULT NULL COMMENT '处理单位名称',
                                           `insert_time` varchar(50) DEFAULT NULL COMMENT '案件/警情录入时间',
                                           `message_content` varchar(500) NOT NULL COMMENT '消息内容',
                                           `read_flag` int(1) DEFAULT '0' COMMENT '已读标志：0-未读，1-已读',
                                           `validflag` int(1) DEFAULT '1' COMMENT '有效标志：0-无效，1-有效',
                                           `addoperator` varchar(50) DEFAULT 'system' COMMENT '添加人',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='案件警情消息提醒表';
...
ALTER TABLE
    ...
    -- 3.4 涉赌前科标记
    ADD
        COLUMN `has_shedu_record` int(1) DEFAULT 0 COMMENT '涉赌前科',
    -- 3.5 手机号码
    ADD
...
CREATE TABLE `p_zaperson_pei_t` (
    ...
    -- 审计字段
                                    ...
                                    `extra1` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT 'extra1',
                                    `extra2` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT 'extra2',
...
-- getAge函数年龄计算修复
-- 修复日期：2026-02-10
-- 问题：15位和18位身份证年龄判断逻辑错误，导致已满18岁的人员仍被标记为未成年
...
-- 创建修复后的函数
DELIMITER ;;
CREATE FUNCTION `getAge`(`cardnumber` varchar(50))
    RETURNS int(11)
BEGIN
    DECLARE age INT;
    ...
    SET cardLen = CHAR_LENGTH(TRIM(cardnumber));
    SET nowYear = YEAR(NOW());
    SET nowMonth = MONTH(NOW());
    SET nowDay = DAY(NOW());

    IF cardLen = 15 THEN
        -- 15位身份证：出生年份只有2位，需要加1900
        SET birthYear = 1900 + CONVERT(SUBSTR(cardnumber, 7, 2), UNSIGNED INTEGER);
        SET birthMonth = CONVERT(SUBSTR(cardnumber, 9, 2), UNSIGNED INTEGER);
        SET birthDay = CONVERT(SUBSTR(cardnumber, 11, 2), UNSIGNED INTEGER);
    ELSEIF cardLen = 18 THEN
        -- 18位身份证：出生年份是4位完整年份
        SET birthYear = CONVERT(SUBSTR(cardnumber, 7, 4), UNSIGNED INTEGER);
        SET birthMonth = CONVERT(SUBSTR(cardnumber, 11, 2), UNSIGNED INTEGER);
        SET birthDay = CONVERT(SUBSTR(cardnumber, 13, 2), UNSIGNED INTEGER);
    ELSE
        -- 身份证格式不正确
        RETURN 0;
    END IF;
    ...
    -- 计算年龄
    SET age = nowYear - birthYear;

    -- 如果今年的生日还没到，年龄减1
    IF nowMonth < birthMonth OR (nowMonth = birthMonth AND nowDay < birthDay) THEN
        SET age = age - 1;
    END IF;
